...
function estUrgent(dateStr: string): boolean {
  const aujourdhui = new Date();
  const demain = new Date();
  demain.setDate(demain.getDate() + 1);
  return dateStr === formaterDate(aujourdhui) || dateStr === formaterDate(demain);
}
...
export default function DashboardScreen({ navigation }: any) {
  ...
  const chargerDonnees = async () => {
    ...
    try {
      ...
      console.error('Erreur lors du chargement des urgences :', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      ToastAndroid.show('Synchronisation avec Google Sheets réussie !', ToastAndroid.SHORT);
    } catch (error) {
      console.error('Échec de la synchronisation manuelle :', error);
      ToastAndroid.show('Échec de la synchronisation.', ToastAndroid.SHORT);
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const renderItem = ({ item }: { item: any }) => {
    const garmentName = getGarmentName(item.garment_type_id);
    const urgente = estUrgent(item.dueDateLabel);
    ...
    return (
      <View style={styles.card}>
        ...
        <View style={styles.cardContent}>
          ...
          <Text style={styles.receivedDate}>Pris le {item.dueDateLabel?.split(' ')[0]}</Text>
        </View>
        ...
        <FlatList
            ...
            <>
              <View style={styles.summaryBanner}>
                ...
                <Text style={styles.bannerText}>
                  <Text style={styles.bannerHighlight}>{commandes.length}</Text> commande{commandes.length > 1 ? 's' : ''} à livrer cette semaine
                </Text>
              </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Icon name="checkmark-done-circle-outline" size={48} color="#10B981" />
              <Text style={styles.emptyTitle}>Tout est à jour</Text>
              <Text style={styles.emptySubtitle}>Aucune commande urgente pour les 7 prochains jours.</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  summaryBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#E0E7FF', borderRadius: 16, padding: 16, marginBottom: 24 },
  bannerIcon: { marginRight: 12 },
  bannerText: { fontSize: 15, color: '#1E293B', fontWeight: '500' },
  bannerHighlight: { fontWeight: 'bold', fontSize: 18 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  sectionTitle: { fontSize: 18, fontWeight: 'bold', color: '#1E293B' },
  seeAllText: { fontSize: 14, color: '#E2583E', fontWeight: 'bold' },
  card: { flexDirection: 'row', backgroundColor: '#FFFFFF', borderRadius: 16, padding: 12, marginBottom: 12, alignItems: 'center', elevation: 2, shadowColor: '#000000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2 },
  ...
  clientName: { fontSize: 16, fontWeight: 'bold', color: '#1E293B', marginBottom: 4 },
  garmentType: { fontSize: 14, color: '#64748B', marginBottom: 4 },
  receivedDate: { fontSize: 11, color: '#94A3B8' },
  badge: { flexDirection: 'row', alignItems: 'center', paddingVertical: 6, paddingHorizontal: 10, borderRadius: 12 },
  badgeUrgent: { backgroundColor: '#E2583E' },
  badgeNormal: { backgroundColor: '#F1F5F9' },
  badgeIcon: { marginRight: 4 },
  badgeText: { fontSize: 12, fontWeight: '600' },
  badgeTextUrgent: { color: '#FFFFFF' },
  badgeTextNormal: { color: '#64748B' },
  emptyContainer: { alignItems: 'center', paddingTop: 40 },
  emptyTitle: { fontSize: 16, fontWeight: 'bold', color: '#1E293B', marginTop: 12, marginBottom: 4 },
  emptySubtitle: { fontSize: 13, color: '#64748B' },
  fab: { position: 'absolute', bottom: 20, right: 20, width: 56, height: 56, borderRadius: 16, backgroundColor: '#E2583E', justifyContent: 'center', alignItems: 'center', elevation: 4, shadowColor: '#000000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.15, shadowRadius: 4 },
});