...
*   **Saisie ultra-rapide des mesures (Zéro Friction) :** Un pavé numérique customisé (de 10 à 190 cm avec un pas de 5) sous forme de panneau coulissant (Bottom Sheet) pour éviter l'ouverture du clavier système et éliminer l'obstruction visuelle. Le layout est désormais adaptatif grâce à `Dimensions.get('window').height` pour garantir la visibilité sur tous les écrans.
*   **Référentiel de vêtements dynamique (10 modèles) :** Gestion de fiches de mesures spécifiques selon le type de vêtement choisi (Sénateur/Ministre, Kaba Ngondo, Grand Boubou, Agbada, Caftan, Costume classique, etc.). Le formulaire est désormais découpé en trois étapes modulaire : Profil Client, Type de Vêtement, et Prise de Mesures.
...
*   **Tableau de bord intelligent (Atelier) :** Affichage des livraisons urgentes de la semaine avec des avatars d'initiales clients et des indicateurs visuels de priorité. Le bouton de synchronisation est désormais relié à la fonction réelle `lancerSynchronisation()` avec un indicateur de chargement.
...
*   **Persistance de session :** `@react-native-async-storage/async-storage` (conservation du jeton Google Sign-In pour éviter la reconnexion à chaque lancement)
...
│   │   └── steps/          # Composants modulaire des étapes du formulaire
│   │       ├── StepProfilClient.tsx
│   │       ├── StepTypeVetement.tsx
│   │       └── StepMesuresSplit.tsx
...
│       ├── authService.ts  # Gestion de session Google Sign-In
